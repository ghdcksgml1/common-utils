...
public class DateDistance {
    ...
    public static String of(Temporal target) {
        LocalDateTime targetDate = castLocalDateTime(target);

        LocalDateTime now = LocalDateTime.now();
        long dateDistance = Duration.between(targetDate, now).getSeconds();

        return judgeRelativeTime(dateDistance);
    }

    private static String judgeRelativeTime(long dateDistance) {
    ...
    private static LocalDateTime castLocalDateTime(Temporal target) {
